...
class AIPlayerGameLogic(BaseGameLogic):
    ...
    def find_move(self, is_alone):
        """
        temp_obstacles is temporary obstacles (snake or other ones)
        """
        # If snake is not alone then need to find path continously
        # If snake don't have a path then need to find a path
        if not is_alone or not self.path:
