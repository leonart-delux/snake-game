...
class AIPlayerGameLogic(BaseGameLogic):
    ...
    def move_towards_food(self):
        if self.x1 < self.foodx:
            self.x1_change = self.snake_block
            self.y1_change = 0
        elif self.x1 > self.foodx:
            self.x1_change = -self.snake_block
            self.y1_change = 0
        if self.y1 < self.foody:
            self.y1_change = self.snake_block
            self.x1_change = 0
        elif self.y1 > self.foody:
            self.y1_change = -self.snake_block
            self.x1_change = 0
...
class MultiplayerGameLogic:
    ...
    def update_multiplay_screen(self):
        ...
        self.ui.draw_food(self.snake1.foodx, self.snake1.foody, self.snake1.snake_block, self.ui.light_blue)  
        self.ui.draw_food(self.snake2.foodx, self.snake2.foody, self.snake2.snake_block, self.ui.light_red)  
        ...
        self.ui.display_text(f"AI: {self.snake2_score}", self.ui.width - 120, 10, self.ui.red, 20)
        
        self.ui.refresh_screen()

        if self.snake1.x1 == self.snake1.foodx and self.snake1.y1 == self.snake1.foody:
            self.snake1.foodx, self.snake1.foody = self.snake1.random_food()
            self.snake1.length_of_snake += 1
            self.snake1_score += 1

        if self.snake2.x1 == self.snake2.foodx and self.snake2.y1 == self.snake2.foody:
